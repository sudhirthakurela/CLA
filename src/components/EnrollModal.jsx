...
export default function EnrollModal({ isOpen, onClose }) {
  ...
  const [form, setForm] = useState({
    ...
    childName: '', childDob: '', center: '', program: '', startDate: '', message: '',
  })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(FORMSPREE, {
        ...
        body: JSON.stringify({
          _subject: `Enrollment Inquiry — ${form.childName}`,
          ...form,
        }),
      })
      if (res.ok) {
  ...
  const handleClose = () => {
    setStatus('idle')
    setStep(1)
    setForm({ parentName: '', email: '', phone: '', childName: '', childDob: '', center: '', program: '', startDate: '', message: '' })
    onClose()
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-4xl shadow-pop w-full max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white rounded-t-4xl px-8 pt-8 pb-4 border-b border-gray-100 z-10">
          <button onClick={handleClose} className="absolute top-6 right-6 p-2 rounded-full hover:bg-gray-100 transition-colors text-gray-400">
            <X size={20} />
          </button>
          <div className="pr-8">
            <h2 className="font-display font-900 text-2xl text-gray-900">
              {status === 'success' ? 'Inquiry Sent! 🎉' : 'Start Enrollment'}
            </h2>
            {status !== 'success' && (
            ...
            <div className="flex items-center gap-2 mt-4">
              {[1, 2].map((s) => (
                <div key={s} className={`h-2 rounded-full transition-all duration-300 ${
                  s === step ? 'bg-brand-coral w-8' : s < step ? 'bg-brand-teal w-4' : 'bg-gray-200 w-4'
                }`} />
              ))}
            </div>
          )}
        ...
        <div className="px-8 py-6">
          {status === 'success' ? (
            <div className="text-center py-6">
              ...
              <button onClick={handleClose} className="bg-brand-coral text-white font-display font-700 px-8 py-3 rounded-2xl hover:bg-coral-600 transition-colors">
                Done
              </button>
            </div>
          ) : (
            <form onSubmit={step === 2 ? handleSubmit : (e) => { e.preventDefault(); setStep(2) }}>
              {status === 'error' && (
                <div className="mb-4 bg-red-50 border border-red-100 rounded-2xl px-5 py-3 text-sm text-red-600 font-display font-600">
              ...
              {step === 1 && (
                <div className="space-y-4">
                  <h3 className="font-display font-700 text-sm text-gray-500 uppercase tracking-wide">About Your Child</h3>
                  <div>
                    <label className="block text-sm font-display font-600 text-gray-700 mb-1.5">Child's Name *</label>
                    <input type="text" name="childName" value={form.childName} onChange={handleChange} required
                  ...
                  <div>
                    <label className="block text-sm font-display font-600 text-gray-700 mb-1.5">Date of Birth *</label>
                  ...
                  <div>
                    <label className="block text-sm font-display font-600 text-gray-700 mb-1.5">Desired Start Date</label>
                    <input type="date" name="startDate" value={form.startDate} onChange={handleChange}
                      className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-brand-coral/30 focus:border-brand-coral text-sm transition-all" />
                  </div>
                  <button type="submit" className="w-full bg-brand-coral text-white font-display font-700 py-3.5 rounded-2xl hover:bg-coral-600 transition-colors mt-2">
              ...
              {step === 2 && (
                <div className="space-y-4">
                  <h3 className="font-display font-700 text-sm text-gray-500 uppercase tracking-wide">Parent / Guardian Info</h3>
                  <div>
                    <label className="block text-sm font-display font-600 text-gray-700 mb-1.5">Your Name *</label>
                  ...
                  <div>
                    <label className="block text-sm font-display font-600 text-gray-700 mb-1.5">Questions or Notes</label>
                  ...
                  <div className="flex gap-3">
                    ...
                    <button type="submit" disabled={status === 'sending'}
                      className="flex-[2] bg-brand-coral text-white font-display font-700 py-3.5 px-6 rounded-2xl hover:bg-coral-600 transition-colors disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2">
                      {status === 'sending' ? (
